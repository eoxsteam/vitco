...
class MultiLotLine(models.Model):
    _name = 'multi.lot.line'
    _description = 'Job Order Multi Lot'
    ...
    def add_product_to_line(self):
        ...
        if self.lot_id:
            if self.job_order_ref_id.operation == 'parting':
                if self.number_of_parts > 0:
                    i = 0
                    while i < self.number_of_parts:
                        self.job_order_ref_id.write({
                            'job_line_ids': [(0, 0, {
                                'lot_id': self.lot_id.id,
                                'product_id': self.job_order_ref_id.order_line_product.id if self.job_order_ref_id.order_line_product else self.product_id.id,
            ...
            elif self.job_order_ref_id.operation == 'cutting':
            ...
            elif self.job_order_ref_id.operation == 'slitting':
                if self.number_of_slits > 0:
                    ...
                    while i < self.number_of_slits:
                        ...
                        i += 1
            elif self.job_order_ref_id.operation == 'multistage':
                self.job_order_ref_id.write({
            ...
            elif self.job_order_ref_id.operation == 'annealing':
                self.job_order_ref_id.write({
                    'job_line_ids': [(0, 0, {
                        'lot_id': self.lot_id.id,
                        'product_id': self.job_order_ref_id.order_line_product.id if self.job_order_ref_id.order_line_product else self.product_id.id,
            ...
            elif self.job_order_ref_id.operation == 'cr':
                self.job_order_ref_id.write({
                    'job_line_ids': [(0, 0, {
                        'lot_id': self.lot_id.id,
                        'product_id': self.job_order_ref_id.order_line_product.id if self.job_order_ref_id.order_line_product else self.product_id.id,
