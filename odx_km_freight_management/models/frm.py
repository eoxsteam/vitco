...
class FreightLine(models.Model):
    ...
    def _get_product_list(self):
        if self.freight_id.stock_picking_id:
            # product_to_transport = self.freight_id.stock_picking_id.mapped('move_line_nosuggest_ids').mapped('product_id').ids
            product_to_transport = self.freight_id.stock_picking_id.mapped('move_ids_without_package').mapped('product_id').ids
            fields_domain = [('id', 'in', product_to_transport)]
            return {'domain': {'product_id': fields_domain, }}
    ...
    def send_email(self):
        self.ensure_one()
        for line in self:
            template_id = self.env.ref('odx_km_freight_management.mail_template_freight_eoxs').id
    ...
    #     for line in self:
    #         total = 0
    #         # for p_line in line.transported_lot_ids:
    #             # if line.freight_id.stock_picking_id:
    #             #     move_line = self.env['stock.move.line'].search(
    #             #         [('picking_id', '=', line.freight_id.stock_picking_id.id), ('product_id', '=', p_line.product_id)])
    #             #     if move_line:
    #             #         # total += lot.product_qty
    #             #         total += move_line.qty_done
    #             #     else:
    #             #         total += p_line.product_qty
    #             # else:
    #         total += p_line.product_qty
    ...
    def _onchange_weight(self):
        for line in self:
            total = 0
            for cargo in line.freight_id.cargo_lines:
                if line.product_id == cargo.product_id:
                    total += cargo.product_uom_qty
            line.total_weight = total
