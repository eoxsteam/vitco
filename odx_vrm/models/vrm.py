...
class VrmLead(models.Model):
    _name = "vrm.lead"
    _description = "Lead/VRM"
    ...
    # def _auto_init(self):
    #     res = super(VrmLead, self)._auto_init()
    ...
    def _default_stage_id(self):
        return self._stage_find(team_id=False, domain=[('fold', '=', False)]).id
        # stages = self.env['vrm.stage'].search([()],order='sequence')
        # if stages:
        #     return stages[0].id
    ...
    def action_new_rfq(self):
        if self.is_multiple_vendor:
            purchase_obj = self.env['purchase.order']

            for rec in self.vrm_vendor_ids:
    ...
    def action_view_purchase_order(self):
        purchase_orders = self.env['purchase.order'].search([('vrm_id', '=', self.id)])
        ...
        action['domain'] = [('vrm_id', '=', self.id), ('state', 'not in', ['cancel'])]
        if self.rfq_count == 1:
    ...
    def _compute_rfq_count(self):
        purchase = self.env['purchase.order'].search([('vrm_id', '=', self.id)])
        for lead in self:
            quotation_cnt = 0
            for order in purchase:
                if order.state not in ('cancel'):
                    quotation_cnt += 1
            lead.rfq_count = quotation_cnt

    name = fields.Char('Opportunity', required=True, index=True)
    is_multiple_vendor = fields.Boolean('Multiple vendor VRM')
    vrm_vendor_ids = fields.Many2many('res.partner', string='Vendors', tracking=10, index=True,
                                      domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]", )
    pr_management = fields.Selection([('crm', 'CRM'), ('vrm', 'VRM')])
    rfq_count = fields.Integer(compute='_compute_rfq_count')
    partner_id = fields.Many2one('res.partner', string='Vendor', tracking=10, index=True,
                                 domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]",
    ...
    tag_ids = fields.Many2many('vrm.lead.tag', 'vrm_lead_tag_rel', 'lead_id', 'tag_id', string='Tags',
    ...
    priority = fields.Selection(vrm_stage.AVAILABLE_PRIORITIES, string='Priority', index=True,
                                default=vrm_stage.AVAILABLE_PRIORITIES[0][0])
    date_closed = fields.Datetime('Closed Date', readonly=True, copy=False)

    stage_id = fields.Many2one('vrm.stage', string='Stage', tracking=True, index=True, copy=False,
                               group_expand='_read_group_stage_ids', default=lambda self: self._default_stage_id())
    # domain="['|', ('team_id', '=', False), ('team_id', '=', team_id)]",,default=lambda self: self._default_stage_id()
    ...
    planned_revenue = fields.Monetary('Expected Cost', currency_field='company_currency', tracking=True)
    ...
    lost_reason = fields.Many2one('vrm.lost.reason', string='Lost Reason', index=True, tracking=True)

    vrm_spec_ids = fields.One2many('product.quotation', 'vrm_reference_id', string="Specs")
    ...
    def _compute_meeting_count(self):
        count = 0
        # meeting_data = self.env['calendar.event'].saerch([('vrm_lead_id', '=', self.id)])

        # meeting_data = self.env['calendar.event'].read_group([('vrm_lead_id', 'in', self.ids)], ['vrm_lead_id'],
        #                                                      [])
        # print(meeting_data, 'map1')
        # mapped_data = {m['vrm_lead_id'][0]: m['opportunity_id_count'] for m in meeting_data}
        # print(mapped_data,'map')
    ...
    def create(self, vals):
        ...
        result = super(VrmLead, self.with_context(context)).create(vals)
    ...
    def write(self, vals):
        ...
        if 'stage_id' in vals:
            ...
            stage_id = self.env['vrm.stage'].browse(vals['stage_id'])
            if stage_id.is_won:
        ...
        write_result = super(VrmLead, self).write(vals)
    ...
    def _update_probability(self):
        ...
        for lead in self:
            if lead.id in lead_probabilities:
                ...
                super(VrmLead, lead).write(proba_vals)
        return

    def _should_update_probability(self, vals):
        fields_to_check = ['tag_ids', 'stage_id'] + self._pls_get_safe_fields()
    ...
    def copy(self, default=None):
        ...
        return super(VrmLead, self.with_context(context)).copy(default=default)

    @api.model
    def _fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        if self._context.get('vrm_lead_id'):
            opportunity = self.browse(self._context['vrm_lead_id'])
        ...
        res = super(VrmLead, self)._fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar,
                                                    submenu=submenu)
        if view_type == 'form':
    ...
    def toggle_active(self):
        res = super(VrmLead, self).toggle_active()
    ...
    def _rebuild_pls_frequency_table_threshold(self):
        ...
        if pls_threshold:
            lead_count = self.env['vrm.lead'].sudo().search_count([])
    ...
    def action_schedule_meeting(self):
        ...
        action['context'] = {
            'default_vrm_lead_id': self.id if self.type == 'opportunity' else False,
            'default_partner_id': self.partner_id.id,
    ...
    def _stage_find(self, team_id=False, domain=None, order='sequence'):
        ...
        return self.env['vrm.stage'].search(search_domain, order=order, limit=1)
    ...
    def _merge_opportunity_attachments(self, opportunities):
        ...
        def _get_attachments(vrm_lead_id):
            return self.env['ir.attachment'].search([('res_model', '=', self._name), ('res_id', '=', vrm_lead_id)])
    ...
    def merge_opportunity(self, user_id=False, team_id=False):
        ...
        if merged_data.get('team_id'):
            team_stage_ids = self.env['vrm.stage'].search(
    ...
    def _get_duplicated_leads_by_emails(self, partner_id, email, include_lost=False):
        ...
        if not include_lost:
            ...
            partner_match_domain += ['|', '&', ('type', '=', 'lead'), ('active', '=', True),
                                     ('type', '=', 'opportunity')]
    ...
    def log_meeting(self, meeting_subject, meeting_date, duration):
        ...
        message = _("Meeting scheduled at '%s'<br> Subject: %s <br> Duration: %s hours") % (
    ...
    def _notify_get_reply_to(self, default=None, records=None, company=None, doc_names=None):
        ...
        if leftover:
            res.update(super(VrmLead, leftover)._notify_get_reply_to(default=default, records=None, company=company,
                                                                     doc_names=doc_names))
        return res
    ...
    def _message_get_suggested_recipients(self):
        recipients = super(VrmLead, self)._message_get_suggested_recipients()
        try:
            for lead in self:
                if lead.partner_id:
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        if msg_dict.get('priority') in dict(vrm_stage.AVAILABLE_PRIORITIES):
        ...
        return super(VrmLead, self).message_new(msg_dict, custom_values=defaults)
    ...
    def _message_post_after_hook(self, message, msg_vals):
        ...
        return super(VrmLead, self)._message_post_after_hook(message, msg_vals)

    def _message_partner_info_from_emails(self, emails, link_mail=False):
        result = super(VrmLead, self)._message_partner_info_from_emails(emails, link_mail=link_mail)
    ...
    def _pls_get_naive_bayes_probabilities(self, batch_mode=False):
        ...
        LeadScoringFrequency = self.env['vrm.lead.scoring.frequency']

        # get stages
        first_stage_id = self.env['vrm.stage'].search([], order='sequence', limit=1)
        won_stage_ids = self.env['vrm.stage'].search([('is_won', '=', True)]).ids
        ...
        result = dict((team_id, dict((field, dict(won_total=0, lost_total=0)) for field in fields)) for team_id in
                      frequency_team_ids)
    ...
    def _rebuild_pls_frequency_table(self):
        ...
        stage_ids = self.env['vrm.stage'].search_read([], ['sequence', 'name', 'id'], order='sequence')
        ...
        self.env['vrm.lead.scoring.frequency'].create(values_to_create)
    ...
    def _update_automated_probabilities(self):
        ...
        leads_to_update = self.env['vrm.lead'].search(pending_lead_domain)
    ...
    def _pls_update_frequency_table(self, values_to_create, stage_ids, stage_sequences, team_id=None):
        ...
        frequencies = self._pls_update_frequency_table_fields(frequencies, stage_ids, stage_sequences, fields, team_id,
                                                              pls_start_date)
    ...
    def _pls_update_frequency_table_fields(self, frequencies, stage_ids, stage_sequences, fields, team_id,
                                           pls_start_date):
