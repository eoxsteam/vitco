...
class Team(models.Model):
    _name = 'vrm.team'
    _inherit = ['mail.alias.mixin', 'vrm.team']
    ...
    def _compute_unassigned_leads_count(self):
        leads_data = self.env['vrm.lead'].read_group([
            ('team_id', 'in', self.ids),
            ('type', '=', 'lead'),
            ('user_id', '=', False),
        ], ['team_id'], ['team_id'])
    ...
    def _compute_opportunities(self):
        opportunity_data = self.env['vrm.lead'].search([
            ('team_id', 'in', self.ids),
            ('probability', '<', 100),
            ('type', '=', 'opportunity'),
    ...
    def _compute_overdue_opportunities(self):
        opportunity_data = self.env['vrm.lead'].read_group([
            ('team_id', 'in', self.ids),
            ('probability', '<', 100),
            ('type', '=', 'opportunity'),
    ...
    def action_your_pipeline(self):
        action = self.env.ref('odx_vrm.vrm_lead_action_pipeline').read()[0]
    ...
    def action_primary_channel_button(self):
        if self.use_opportunities:
            return self.env.ref('odx_vrm.vrm_case_form_view_salesteams_opportunity').read()[0]
